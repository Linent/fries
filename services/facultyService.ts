...
export const getFaculties = async (): Promise<Faculty[]> => {
  try {
    ...
    throw new Error(error.response?.data?.message || "Error al obtener facultades");
  }
};

export const createFaculty = async (facultyData: Partial<Faculty>): Promise<Faculty> => {
  try{
    const response = await api.post(`${BACKEND_URL}/${facultyPath}`, facultyData, {
     headers: getAuthHeaders(),
    });
...
export const updateFaculty = async (id: string, facultyData: Partial<Faculty>): Promise<Faculty> => {
  try {
    const response = await api.put(`${BACKEND_URL}/${facultyPath}/${id}`, facultyData, {
      headers: getAuthHeaders(),
    });
    return response.data;
  } catch (error: any) {
    console.error("Error al actualizar la facultad:", error);
    throw new Error(error.response?.data?.message || "Error al actualizar facultad");
  }
};
...
export const toggleFacultyActive = async (id: string): Promise<any> => {
  try {
    const response = await api.post(`${BACKEND_URL}/${facultyPath}/toggle/${id}`, {}, {
      headers: getAuthHeaders(),
    });
    return response.data;
  } catch (error: any) {
